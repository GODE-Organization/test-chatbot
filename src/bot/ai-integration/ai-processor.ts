  ...
  public async sendMessageToAI(
    ...
    try {
      ...
      for (const action of result.actions || []) {
        ...
        // Si es una consulta de garantías exitosa, enviar los datos a Gemini para formatear
        if (action.command === 'CONSULT_GUARANTEES' && actionResult.success && actionResult.data) {
          ...
          const guaranteesResult = await this.sendGuaranteesDataToGemini(
            actionResult.data,
            userId,
            sessionData
          )
...
• **Cafetera de Goteo** - Precio: $27 USD / 5,272 Bs
  ...
  /**
   * Envía datos de garantías a Gemini para formateo
   */
  private async sendGuaranteesDataToGemini(
    guarantees: any[],
    userId: number,
    sessionData?: Record<string, any>
  ): Promise<MessageProcessingResult> {
    try {
      // Crear prompt específico para formatear garantías
...
Puedes hacerlo enviando el comando /garantia o usando el botón de abajo.
      ...
      return {
        success: true,
        response: result.response || { text: 'Garantías formateadas', parse_mode: 'Markdown' as const },
        actions: result.actions || [],
        session_data: result.session_data || {}
      }
