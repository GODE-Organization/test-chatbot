...
async function main(): Promise<void> {
  // Crear servidor HTTP PRIMERO para que Render detecte el puerto
  ...
  // Manejar señales de terminación (registrar ANTES de inicializar el bot)
  process.once('SIGINT', () => {
    server.close()
    if (bot) bot.shutdown(0, 'SIGINT')
  })
  process.once('SIGTERM', () => {
    server.close()
    if (bot) bot.shutdown(0, 'SIGTERM')
  })
  ...
  try {
    ...
    // No hacer process.exit(1) aquí para que el servidor HTTP siga funcionando
  }
}
