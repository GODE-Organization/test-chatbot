 ...
 */
export async function handleSettingsCallbacks(ctx: BotContext) {
  try {
...
/**
 * Manejar callbacks de idioma
 */
export async function handleLanguageCallbacks(ctx: BotContext) {
  try {
    if (!ctx.callbackQuery || !('data' in ctx.callbackQuery)) return
    ...
    if (data in languageMap) {
      ...
      if (lang) {
        // Aquí guardarías el idioma en la base de datos
        // await userModel.updateUserLanguage(ctx.from?.id, lang.code)
 ...
 */
export async function handleNotificationCallbacks(ctx: BotContext) {
  try {
    if (!ctx.callbackQuery || !('data' in ctx.callbackQuery)) return
    ...
    if (data in notificationMap) {
      ...
      // await userModel.updateUserNotificationSettings(ctx.from?.id, data)
 ...
 */
export async function handleHelpCallbacks(ctx: BotContext) {
  try {
    if (!ctx.callbackQuery || !('data' in ctx.callbackQuery)) return
