...
export function getConfirmationKeyboard(action) {
    return Markup.inlineKeyboard([
        [
...
export function createCustomKeyboard(config) {
    const buttons = config.map(row => row.map(button => {
        if (button.url) {
...
export function createCustomInlineKeyboard(config) {
    const buttons = config.map(row => row.map(button => {
        if (button.url) {
            return Markup.button.url(button.text, button.url);
        }
